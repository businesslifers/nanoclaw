...
- `to` is the destination's local-name as registered in your `agent_destinations`. **Inter-agent destinations are not wired on this install yet for you** — your destinations today are channel-only (your own Slack DM with Raels). To check what's wired, ask Raels.
- Other agents on this install — `clientmate`, `marketingteam`, `crm`, `project-management-team`, `dm-with-adam` (Adam's Janet) — exist as agent groups but you cannot `send_message` to them until destination rows are added on your side.
