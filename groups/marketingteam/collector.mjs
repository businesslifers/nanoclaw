...
async function createAdsClient() {
  ...
  // REST transport: the OneCLI proxy TLS-intercepts and only negotiates http/1.1,
  // which breaks gRPC/h2 (SSL alert 120). We call the Google Ads REST endpoint
  // directly with the bearer token above. No gRPC client needed; return a truthy
  // placeholder so the 6 queryAds(adsClient, ...) call sites stay unchanged.
  return { transport: 'rest' };
}

// Google Ads REST returns camelCase JSON, but every downstream reader in this
// file uses the proto snake_case field names. Normalise so consumers are untouched.
...
async function main() {
  ...
  if (adsClient && typeof adsClient.close === 'function') await adsClient.close();
