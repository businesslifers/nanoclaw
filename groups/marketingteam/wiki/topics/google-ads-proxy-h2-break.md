...
- **2026-06-17 RESOLVED — see banner at top of page.** Real root cause was a stale `login-customer-id: 3218082250` header in `collector.mjs queryAds()`, NOT an infra/proxy issue. Adam removed the header host-side; Jun 17 run returned 7 succeeded, 0 failed (first clean run since Jun 6, ending an 11-day outage). No infra host-exemption existed or was needed. The "await infra exemption" conclusion above (Jun 5–12) was a misdiagnosis. Full pipeline ran end-to-end and posted the daily report.

## Related open issue (separate) — now has its own page
**Month-end MTD date bug:** on the last day of a month the collector computes MTD start as the *next* month's 1st → GA4 rejects the inverted range, all 7 accounts fail. Confirmed May 31 and again Jun 30 2026; self-clears on the 1st. Recurs every month-end until fixed. Full detail, runbook, and fix moved to its own page: [collector-month-end-mtd-bug.md](collector-month-end-mtd-bug.md).
