...
- **Role:** connects to Google Ads API and GA4 for all 7 active clients; pulls campaign performance, search terms, ad copy, disapproved ads, and website metrics across three time periods
...
- **Output:** Slack mrkdwn message posted to the marketingteam channel
...
**Slack note:** report uses Slack mrkdwn syntax (`:emoji:` shortcodes, `*bold*`, `_italic_`, `•` bullets). If formatting is garbled, check for `[text](url)` link syntax leaking through — Slack requires `<url|text>` instead.
