...
// 1. Direct calls from ads (AD_CALL type only — calls placed through the ad itself)
...
// Filter to AD_CALL category only (direct calls through the ad)
// Need conversion action type — filter by action name 'Calls from ads'
const callsByCampaign = {};
for (const row of callRows) {
  const actionName = row.segments?.conversion_action_name || '';
  if (actionName !== 'Calls from ads') continue;
...
// 2. Contact page hits by campaign (GA4)
...
// 3. Contact form submissions by campaign (GA4 — Submit events only, not View)
...
console.log(JSON.stringify(pageHits.rows || [], null, 2));

console.log('\n=== FORM SUBMISSIONS ===');
console.log(JSON.stringify(formSubs.rows || [], null, 2));

console.log('\n=== CALLS FROM ADS (direct) ===');
console.log(JSON.stringify(callsByCampaign, null, 2));
