...
Each `/add-<name>` skill is idempotent: `git fetch <remote> <branch>` → copy module(s) into the standard paths → append a self-registration import to the relevant barrel → `pnpm install <pkg>@<pinned-version>` → build. Channel skills carry these steps as `nc:` directive fences: setup applies them via the engine (`scripts/skill-apply.ts`), an agent applies the prose — same install either way. See [docs/skill-directives.md](docs/skill-directives.md).

**This install's `origin` has neither branch** (it's a disaster-recovery mirror, not the framework repo). Confirmed sources here: `private/channels` for channels (canonical — carries local patches upstream/channels lags), `upstream/providers` for providers (`private` has no `providers` branch). Don't trust a skill's hardcoded `git fetch origin <branch>` at face value on this install — check which remote actually has the branch first. `.env` sets `NANOCLAW_CHANNELS_REMOTE=private` so the skill-apply engine's `from-branch` resolution lands on `private` — but that override applies to **every** branch, so an engine-driven apply of a `providers`-branch skill (e.g. codex) would try `git fetch private providers` and fail; apply provider skills by hand from `upstream/providers` (or unset the var for that run).

**`private/channels` can also lag `upstream/channels`.** "Local file identical to private/channels" does not mean current — check `git merge-base --is-ancestor upstream/channels private/channels`. If private lags, merge upstream/channels INTO private/channels (scratch worktree; resolve conflicts as unions preserving local patches like setReaction and the newer version pins), push `private channels`, then copy from the synced branch. Done 2026-07-13 for channel-defaults + Slack Socket Mode.

**Merging upstream (`/update-nanoclaw`) — recurring hazards:**
...
- Migration number collisions: upstream and local both assign sequential numbers to `src/db/migrations/*.ts`, and this fork has already renumbered some (local 016–021 = dashboard-audit / agent-group-hidden-dashboard / messaging-group-instance / agent-message-policies / approvals-approver-user-id / wiring-threads — the last three renumbered from upstream's 017–019, so local numbering runs +2 ahead of upstream; skill-installed migrations can land at arbitrary numbers, e.g. `103-work-items.ts`). The runner dedupes by `name`, not `version` (`src/db/migrations/index.ts`), so a *textually clean* merge can still land two files exporting the same `migrationNNN` symbol — no conflict markers, but `tsc` fails on the duplicate identifier. Renumber the incoming migration to the next free number and append it to the `migrations` array in `index.ts` before building.
...
- Group wikis and conversation archives under `groups/` churn constantly (agents write them at runtime) and are tracked. When a clean-tree preflight is blocked by them, commit them as a `chore: sync live install state` commit (the established convention) — don't stash agent-written state.
...
On THIS install, when the host first spawns a session for a new agent group, `container-runner.ts` calls `onecli.ensureAgent({ name, identifier })` and the OneCLI `POST /api/agents` endpoint creates the agent in **`selective`** secret mode — meaning **no secrets are assigned to it by default**, even if the secrets exist in the vault and have host patterns that would otherwise match. (Upstream docs claim the default is now `all`; that has not been the observed behavior here, so verify with `onecli agents list` rather than assuming.)
...
Host tests that exercise group-creation paths (`initGroupFilesystem`) must mock `GROUPS_DIR` in `config.js`, not just `DATA_DIR` — otherwise each `pnpm test` run scaffolds stray `groups/<fixture-name>/` dirs into the real groups directory (and trips clean-tree preflights). Upstream tests have shipped with this bug (fixed locally in `channel-approval.test.ts`, 2026-07-13).
...
- **Changing the Dockerfile entrypoint or the dynamic-spawn command** (look for `exec bun run /app/src/index.ts` in `src/container-runner.ts`) → keep `exec bun ...` so signals forward cleanly. The image has no `/app/dist`; don't reintroduce a tsc build step.
