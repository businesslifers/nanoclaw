...
13. **Change team model** — pencil-edit on the Model cell (list view) and the Identity Model row (detail view, which also updates the hero sub-line). Opens an inline form: a provider-aware preset `<select>` (claude → opus/sonnet/haiku/opus[1m]/sonnet[1m]; codex → gpt-5.4/gpt-5.4-mini; other providers get only "default") plus a "custom…" option revealing a free-text input — model strings are opaque to the host (no allowlist; SDK conventions like `sonnet[1m]` pass through), and "default" clears the override. Submits `PATCH /api/agent-groups/:id/model` with `{ model, restart }` → `updateAgentGroupModel` mutator: validates (trim, ≤120 chars, no control chars), authorizes via `hasAdminPrivilege` (same gate as rename), writes `agent_groups.model` + `agent_group.set_model` audit row in one transaction, nudges the pusher. **Column precedence**: the runtime resolver reads `sessions.model → agent_groups.model → container_configs.model → SDK default`, so this override outranks `container.json`'s model but a (theoretical) session-level override survives it; clearing to "default" only clears the group override, so the displayed effective model may fall back to the `container_configs.model` value — expected, not a bug. **Restart semantics**: the model lands in `AGENT_MODEL` env at container spawn only, so by default (checkbox off) a change takes effect on the group's next natural spawn. Ticking "restart now" (with a confirm dialog) kills the group's running/idle containers `update_agent`-style — no respawn message; the next routed message spawns fresh with the new model. In-flight work in those containers is interrupted; there is no heartbeat guard.
...
Expect: container-stats 20 tests, dashboard-tasks 9 tests, dashboard-mutators 48 tests — all passing.
