...
12. **Work Items page** — new `/dashboard/work-items` route over the host's cross-team work-item store (central `work_items` table, migration `103-work-items`): one fetched dataset, three switchable views — **Kanban** (columns = the fixed status enum `open | in_progress | blocked | done | cancelled`, click-to-move as the primary touch-friendly interaction with native drag layered on top calling the same `moveItem()` path), **List** (grouped by owner team, sorted by due date, overdue rows highlighted), and **Timeline** (CSS-grid week/day calendar; chips positioned by due date, with delivered — `completed_at` — vs planned vs overdue distinguished). Full CRUD via a side drawer: create, edit, reassign (team or human assignee), cancel, and append notes; every mutation runs through host mutators (`src/dashboard-work-items-mutators.ts`) with `canAccessAgentGroup` on the item's owner-OR-assignee, an additional target-team check on reassignment, audit rows (`target_type='work_item'`), projection refresh + wake-matrix notification of affected teams, and `nudgePusher`. The `/api/work-items` read route filters per viewer by owner OR assignee (not the single-ID check `/api/tasks` uses — a single-ID filter would hide cross-team delegations from the assignee's own admin). Shared render helpers across all three views: deterministic gradient team avatars, circular-initials human badges, an "owner → assignee" flow indicator, kind badges, and overdue/soon due chips. **Host prerequisite**: the work-items feature in trunk (present since 2026-07-02 — `src/db/work-items.ts`, `src/modules/work-items/`, `src/dashboard-work-items.ts`, `src/dashboard-work-items-mutators.ts`, migration 103, and the agent-side MCP tools). This skill's patch only ships the dashboard-package side; the host side arrives with the tree via git, not from skill resources.
...
The pusher copy also collects `work_items` into the snapshot via `collectWorkItems()` from `src/dashboard-work-items.ts` — that file (and the rest of the work-items feature) ships with the main tree (since 2026-07-02), not with this skill. On an older tree that lacks it, the copied pusher will not compile; update the install first.
...
# NOTE: the mutator bundle now includes the work-item mutators (createWorkItem /
# updateWorkItem / reassignWorkItem / cancelWorkItem / addWorkItemNote), imported
# from src/dashboard-work-items-mutators.ts — part of the main tree since
# 2026-07-02, not a skill resource. On an older tree, update the install first.
