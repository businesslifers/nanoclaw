...
13. **Change team model** — pencil-edit on the Model cell (list view) and the Identity Model row (detail view, which also updates the hero sub-line). Opens an inline form: a provider-aware preset `<select>` plus a "custom…" option revealing a free-text input. Presets come from the pusher snapshot via `src/dashboard-model-presets.ts` (`model_presets` on each group row, carried to the widget through `data-model-presets` on the pencil button): claude → the floating agent-sdk aliases (opus/sonnet/haiku/fable/opus[1m]/sonnet[1m], resolved per family at spawn by the *image's* baked SDK alias table) **plus** explicit current-generation ids (claude-opus-5/claude-sonnet-5/claude-fable-5/claude-haiku-4-5). The aliases only float as far as the baked SDK: on claude-agent-sdk 0.3.197 `opus` still resolves to `claude-opus-4-8`, so picking Opus 5 means picking the explicit `claude-opus-5` — the explicit ids pass through opaquely and are the only way to reach a model the baked alias table doesn't know yet. Refresh `CLAUDE_PRESETS` on each model release; codex → refreshed live from `codex debug models` (the codex picker's own catalog; slugs filtered to `visibility:"list"`, sorted by catalog priority, cached 1h, non-blocking with a static `CODEX_FALLBACK` list — the 5.6/5.5/5.4 tiers, mirroring the catalog as of 2026-07-24 — served only if the CLI is missing or errors — the bin is resolved via PATH then `~/.nvm/versions/node/*/bin/codex` since the service PATH lacks nvm); other providers get only "default". A static `MODEL_PRESETS` table in the UI remains as fallback for snapshots that predate the field — model strings are opaque to the host (no allowlist; SDK conventions like `sonnet[1m]` pass through), and "default" clears the override. Submits `PATCH /api/agent-groups/:id/model` with `{ model, restart }` → `updateAgentGroupModel` mutator: validates (trim, ≤120 chars, no control chars), authorizes via `hasAdminPrivilege` (same gate as rename), writes `agent_groups.model` + `agent_group.set_model` audit row in one transaction, nudges the pusher. **Column precedence**: the runtime resolver reads `sessions.model → agent_groups.model → container_configs.model → SDK default`, so this override outranks `container.json`'s model but a (theoretical) session-level override survives it; clearing to "default" only clears the group override, so the displayed effective model may fall back to the `container_configs.model` value — expected, not a bug. **Restart semantics**: the model lands in `AGENT_MODEL` env at container spawn only, so by default (checkbox off) a change takes effect on the group's next natural spawn. Ticking "restart now" (with a confirm dialog) kills the group's running/idle containers `update_agent`-style — no respawn message; the next routed message spawns fresh with the new model. In-flight work in those containers is interrupted; there is no heartbeat guard. **Effort**: the same inline form carries a reasoning-effort `<select>` ("default effort" + provider vocab: claude → low/medium/high/xhigh/max per the agent-sdk `EffortLevel` type, default `high` when unset; codex → none/minimal/low/medium/high/xhigh, the `model_reasoning_effort` values). Presets ride the snapshot as `effort_presets` (static per provider — `getEffortPresets` in `src/dashboard-model-presets.ts`; a UI-side `EFFORT_PRESETS` table is the fallback for older snapshots), the current value as `effort` (also on `container_config`). Unlike model, effort has **no resolution ladder** — it lives only in `container_configs.effort` (materialized into `container.json` at spawn), so the mutator writes that table (`ensureContainerConfig` + `updateContainerConfigScalars`) and appends a separate `agent_group.set_effort` audit row; the PATCH body's `effort` field is tri-state (absent = leave untouched, so older callers that only send `model` never clobber it; null/'' = clear to provider default; string = set, trimmed + lowercased, ≤40 chars). Same spawn-time-only semantics as model: without "restart now" the change waits for the next natural spawn.
