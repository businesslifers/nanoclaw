...
/**
 * Factory for the canSenderInteract predicate.
 * Determines whether a message sender is allowed to interact with the agent.
 */
export function createCanSenderInteract(
  chatJid: string,
  isMainGroup: boolean,
  group: { trigger?: string; requiresTrigger?: boolean },
): (msg: NewMessage) => boolean {
  return (msg) => {
...
export async function handleSessionCommand(opts: {
  ...
  const cmdMsg = missedMessages.find((m) => {
    command = extractSessionCommand(m.content, triggerPattern);
    return command !== null;
  });
  ...
  // No XML formatting — SDK recognizes bare slash commands.
