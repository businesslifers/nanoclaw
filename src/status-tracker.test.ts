...
function makeDeps() {
  return {
    ...
    isTrackedGroup: vi.fn<StatusTrackerDeps['isTrackedGroup']>(
      (jid) => jid === 'main@s.whatsapp.net' || jid === 'group@g.us',
    ),
...
describe('StatusTracker', () => {
  ...
  describe('registered group gating', () => {
    it('ignores messages from unregistered groups', async () => {
      tracker.markReceived('msg1', 'unknown@g.us', false);
      await tracker.flush();
      expect(deps.sendReaction).not.toHaveBeenCalled();
    });

    it('tracks messages from registered non-main groups', async () => {
      tracker.markReceived('msg1', 'group@g.us', false);
      await tracker.flush();
      expect(deps.sendReaction).toHaveBeenCalledTimes(1);
      expect(deps.sendReaction.mock.calls[0][2]).toBe('\u{1F440}');
    });
  });
  ...
  describe('multi-group heartbeat', () => {
    it('fails messages in multiple chats when both containers are dead', async () => {
      deps.isContainerAlive.mockReturnValue(false);

      // Track messages in two different groups
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      tracker.markThinking('msg1');
      tracker.markReceived('msg2', 'group@g.us', false);
      tracker.markThinking('msg2');

      tracker.heartbeatCheck();
      await tracker.flush();

      // Both chats should get ❌ reactions
      const failCalls = deps.sendReaction.mock.calls.filter(
        (c) => c[2] === '❌',
      );
      expect(failCalls).toHaveLength(2);

      // Both chats should get error messages
      expect(deps.sendMessage).toHaveBeenCalledWith(
        'main@s.whatsapp.net',
        '[system] Task crashed — retrying.',
      );
  ...
  describe('history ring buffer', () => {
    ...
    it('records completed message with correct timing', async () => {
      vi.useFakeTimers();
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      ...
      expect(history[0].messageId).toBe('msg1');
      expect(history[0].chatJid).toBe('main@s.whatsapp.net');
      expect(history[0].terminal).toBe('done');
      expect(history[0].durationMs).toBe(5000);
    });

    it('records failed messages', async () => {
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      tracker.markFailed('msg1');
      await tracker.flush();
      ...
      const history = tracker.getHistory();
      expect(history).toHaveLength(1);
      expect(history[0].terminal).toBe('failed');
    });

    it('evicts oldest when exceeding capacity of 10', async () => {
      for (let i = 0; i < 11; i++) {
        tracker.markReceived(`msg${i}`, 'main@s.whatsapp.net', false);
        tracker.markDone(`msg${i}`);
      }
      await tracker.flush();
      ...
      expect(history[0].messageId).toBe('msg1');
      expect(history[9].messageId).toBe('msg10');
    });

    it('receivedAt is not overwritten by markThinking', async () => {
      vi.useFakeTimers();
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      ...
      const history = tracker.getHistory();
      expect(history[0].durationMs).toBe(10000);
    });
  ...
  describe('activity tracking', () => {
    it('setActivity stores and appears in snapshot', () => {
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      tracker.setActivity('main@s.whatsapp.net', '[msg #1] type=assistant');

      const snap = tracker.getSnapshot();
      expect(snap[0].activity).toBe('[msg #1] type=assistant');
    });

    it('markAllDone clears activity', () => {
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      tracker.setActivity('main@s.whatsapp.net', 'working...');
      tracker.markAllDone('main@s.whatsapp.net');

      // Activity should be cleared even though message is cleaned up later
      tracker.markReceived('msg2', 'main@s.whatsapp.net', false);
      const snap = tracker.getSnapshot();
      const msg2 = snap.find((s) => s.messageId === 'msg2');
      expect(msg2?.activity).toBeUndefined();
    });

    it('markAllFailed clears activity', () => {
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      tracker.setActivity('main@s.whatsapp.net', 'working...');
      tracker.markAllFailed('main@s.whatsapp.net', 'crashed');
    ...
    it('snapshot shows undefined activity when none set', () => {
      tracker.markReceived('msg1', 'main@s.whatsapp.net', false);
      const snap = tracker.getSnapshot();
