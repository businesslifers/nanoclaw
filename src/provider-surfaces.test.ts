...
describe('initGroupFilesystem agent surfaces', () => {
  it('writes the default surfaces when no provider is given (today’s behavior)', () => {
    ...
    // appended after the seed (see group-init.ts). Seed still leads the file.
    expect(fs.readFileSync(path.join(groupDir, 'CLAUDE.local.md'), 'utf-8')).toMatch(/^hello\n\n## Communication/);
...
describe('initGroupFilesystem deferred seed (.seed.md)', () => {
  ...
  it('places .seed.md into CLAUDE.local.md for the default provider, then consumes it', () => {
    ...
    // Local customization: DEFAULT_COMMUNICATION_GUIDANCE is appended after the
    // placed seed (see group-init.ts). Seed still leads; .seed.md is consumed.
    expect(fs.readFileSync(path.join(groupDir, 'CLAUDE.local.md'), 'utf-8')).toMatch(
      /^seeded identity\n\n## Communication/,
    );
